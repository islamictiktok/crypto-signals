...
# ==========================================
# 3. محرك الاستراتيجيات (The Classic 6-Power Engine)
# ==========================================
...
class TradingSystem:
    ...
    def _log_trade_result(self, result_type, roe_val, strat_name):
        ...
        if strat_name not in self.stats['strats']:
            self.stats['strats'][strat_name] = {"wins": 0, "losses": 0, "break_evens": 0, "total_roe": 0.0}
            
        self.stats['strats'][strat_name][result_type] += 1
        
        self.stats['all_time']['total_roe'] += roe_val
        self.stats['daily']['total_roe'] += roe_val
        self.stats['strats'][strat_name]['total_roe'] += roe_val
    ...
    async def analyze_btc_trend(self):
        try:
            ...
            if st_df is not None and not st_df.empty:
                ...
                df['st_dir'] = st_df[dir_col[0]] if dir_col else 0
            else:
                df['st_dir'] = 0
            ...
            st_bullish = btc['st_dir'] == 1
            st_bearish = btc['st_dir'] == -1
            ema_bullish = btc['close'] > btc['ema21']
            ema_bearish = btc['close'] < btc['ema21']
            adx_accelerating = btc['adx'] > 20 and btc['adx'] > btc_prev['adx']
            
            if st_bullish and ema_bullish and adx_accelerating and btc['rsi'] < 75:
                return "BULLISH"
            elif st_bearish and ema_bearish and adx_accelerating and btc['rsi'] > 25:
                return "BEARISH"
    ...
    async def execute_trade(self, trade):
        try:
            ...
            msg = (
                ...
                f"⚖️ <b>Leverage:</b> <b>{lev}x</b> (Volatility Adj.)\n"
                f"────────────────\n"
                f"{targets_msg}"
                f"────────────────\n"
                ...
                for sym, trade in list(self.active_trades.items()):
                    ...
                    if price_reached_next_tp:
                        ...
                        if highest_tp_hit == 10: 
                            ...
                            msg = f"🏆 <b>ALL 10 TARGETS SMASHED! (+{tp_roe:.1f}% ROE)</b> 🏦\nTrade Completed."
            ...
            if self.stats.get('strats'):
                for s_name, s_data in self.stats['strats'].items():
                    ...
                    if s_trades > 0:
                        ...
                        strats_msg += f"▪️ Strategy {s_name[:3]}: {s_wr:.0f}% WR | {s_avg_roe:+.1f}% ROE\n"
