...
class Config:
    ...
    VERSION = "V9903.0" # 👈 النسخة المحدثة بنسب الـ ROE
...
# ==========================================
# 3. محرك الاستراتيجيات (The 6-Power Engine)
# ==========================================
class StrategyEngine:
    ...
    def analyze_mtf(symbol, h1_data, m5_data):
        try:
            ...
            h1 = df_h1.iloc[-2]; h1_atr = float(h1['atr'])
            ...
            if market_regime == "TREND":
                ...
                if df_h1.iloc[-3]['rsi'] < 28 and h1['rsi'] > df_h1.iloc[-3]['rsi'] and m5['close'] > m5['ema21']:
                    v_setups.append((3, "Macro Reversal", "LONG"))
                if df_h1.iloc[-3]['rsi'] > 72 and h1['rsi'] < df_h1.iloc[-3]['rsi'] and m5['close'] < m5['ema21']:
                    v_setups.append((3, "Macro Reversal", "SHORT"))
            else:
                if df_h1.iloc[-3]['rsi'] < 40 and h1['rsi'] > df_h1.iloc[-3]['rsi'] and h1['macd_h'] > 0:
                    v_setups.append((4, "Double Bottom (Range)", "LONG"))
                if df_h1.iloc[-3]['rsi'] > 60 and h1['rsi'] < df_h1.iloc[-3]['rsi'] and h1['macd_h'] < 0:
            ...
            step_f = 0.3 if h1['adx'] > 35 else (0.5 if h1['adx'] > 25 else 0.8)
...
# ==========================================
# 4. مدير النظام (Execution Engine)
# ==========================================
class TradingSystem:
    def __init__(self):
    ...
    async def execute_trade(self, trade):
        ...
        # 👈 حساب الـ ROE لكل هدف من الأهداف العشرة
        tp_lines = []
        ...
        if msg_id:
            ...
            self.active_trades[sym] = trade; Log.print(f"🚀 {trade['strat']} FIRED: {sym}", Log.GREEN)
    ...
    async def scan_market(self):
        while self.running:
            ...
            Log.print(f"🔍 [RADAR] Pairs: {len(self.cached_valid_coins)} | BTC: {btc}", Log.BLUE)
        ...
        while self.running:
            for sym, trade in list(self.active_trades.items()):
                ...
                if (trade['side'] == "LONG" and price <= trade['sl']) or (trade['side'] == "SHORT" and price >= trade['sl']):
                    await self.tg.send("🛑 Closed at SL", trade['msg_id']); del self.active_trades[sym]; continue
                step = trade['step']
                if step < 10:
                    tp = trade['tps'][step]
                    if (trade['side'] == "LONG" and price >= tp) or (trade['side'] == "SHORT" and price <= tp):
                        trade['step'] += 1
                        roe = StrategyEngine.calc_actual_roe(trade['entry'], tp, trade['side'], trade['leverage'])
                        if trade['step'] == 1: trade['sl'] = trade['entry']; txt = f"✅ <b>TP1 HIT! (+{roe:.1f}% ROE)</b>\n🛡️ Move SL to Entry"
                        else: trade['sl'] = trade['tps'][trade['step']-2]; txt = f"🔥 <b>TP{trade['step']} HIT! (+{roe:.1f}% ROE)</b>\n📈 SL Trailed"
...
async def lifespan(app: FastAPI):
    await bot.tg.start(); await bot.exchange.load_markets()
