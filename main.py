...
# ✅ الإضافة 1: فلتر السيولة المعقول (5 مليون دولار)
...
async def root():
    ...
    <html>
        <body style='background:#111;color:#00e5ff;text-align:center;padding-top:50px;font-family:sans-serif;'>
            ...
            <p>Filter: Vol > $5M + Copy Paste Enabled</p>
        </body>
    </html>
    """
...
async def get_signal_logic(symbol):
    try:
        # ✅ الإضافة 1 (تكملة): التحقق من السيولة أولاً
    ...
    async with sem:
        ...
        if logic_res:
            ...
            if key not in app_state.sent_signals or (time.time() - app_state.sent_signals[key]) > 21600:
                ...
                # جلب السعر الحي للتأكيد (لأننا جلبناه سابقاً في الفلتر، نحدثه الآن للدخول)
                ticker = await exchange.fetch_ticker(symbol)
                ...
                # ✅ الإضافة 2: تصميم الرسالة الاحترافي (نسخ سهل)
...
async def start_scanning(app_state):
    print(f"🚀 Connecting to KuCoin Futures (SMC Sweep + Vol Filter)...")
    try:
