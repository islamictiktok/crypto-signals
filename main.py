...
# ✅ التعديل 1: فلتر السيولة (5 مليون دولار)
    ...
    async with sem:
        ...
        if logic_res:
            ...
            if key not in app_state.sent_signals or (time.time() - app_state.sent_signals[key]) > 21600:
                ...
                # ✅ التعديل 2: تصميم الرسالة النظيفة (بدون عنوان) والقابلة للنسخ
...
async def start_scanning(app_state):
    ...
    try:
        ...
        # ✅ التعديل 3: الفلترة المرئية (Visible Filtering)
        print(f"🔎 Filtering {len(all_symbols)} pairs for Vol > $5M...", end='\r')
        ...
        print(f"✅ Active Pairs (Vol > 5M): {len(filtered_symbols)} / {len(all_symbols)}")
