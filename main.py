...
# ==========================================
sem = asyncio.Semaphore(20) # 🚨 زيادة الخيوط لـ 20 لزيادة السرعة 
class DataManager:
    def __init__(self):
        self.active_trades = {}
...
async def safe_check(symbol):
    async with sem:
        await asyncio.sleep(0.05) # 🚨 تقليل وقت الراحة لتسريع الفحص
        try:
            # 🚨 بروتوكول الهروب الزمني: عدم انتظار أي عملة تعلق لأكثر من 7 ثوانٍ
    ...
    try:
        ...
        while True:
            if len(app_state.active_trades) >= MAX_TRADES_AT_ONCE:
                ...
                await asyncio.sleep(10); continue # 🚨 تقليل وقت النوم إلى 10 ثواني
            ...
            try:
                ...
                for sym, data in tickers.items():
                    if 'USDT' in sym and ':' in sym: 
                        # 🚨 فلتر العملات الميتة لتسريع الفحص
                ...
                if valid_signals:
                    ...
                    await asyncio.sleep(15) # 🚨 تقليل وقت إعادة البحث من 180 إلى 15 ثانية
            except: await asyncio.sleep(5)
            
            # 🚨 تنظيف الرام هنا مرة واحدة فقط لتخفيف الحمل على المعالج
            gc.collect()
...
async def lifespan(app: FastAPI):
    ...
    # 🚨 تعريف المتغيرات هنا لمنع السيرفر من الانهيار (Status 3 Crash Fix)
