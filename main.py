...
class TrendAnalyst:
    ...
    def analyze(frames: Dict[str, pd.DataFrame]) -> AnalystResult:
        ...
        for tf in ("4h", "1h", "30m"):
            ...
            if c > e20 > e50 > e200:
                votes.append("LONG")
                reasons.append(f"{tf}: السعر فوق EMA20>EMA50>EMA200")
            elif c < e20 < e50 < e200:
                votes.append("SHORT")
                reasons.append(f"{tf}: السعر تحت EMA20<EMA50<EMA200")
            else:
                votes.append("NEUTRAL")
                reasons.append(f"{tf}: ترتيب المتوسطات غير مكتمل")
...
class MomentumAnalyst:
    name = "Momentum & Volatility Analyst"
    weight = 0.25
    ...
    def analyze(frames: Dict[str, pd.DataFrame]) -> AnalystResult:
        ...
        elif short_ok and volatility_ok:
...
class ConsensusEngine:
    ...
    @staticmethod
    def decide(results: List[AnalystResult], min_confidence: float) -> Optional[Consensus]:
        if len(results) != 4:
            return None
        ...
        reasons = [
            f"{counts[direction]}/4 محللين في اتجاه {direction}",
...
class GoldenConsensusEngine:
    ...
    def analyze(
        ...
        consensus = ConsensusEngine.decide(results, CFG.min_confidence)
        if not consensus:
            return None
        ...
        async def analyze_symbol(symbol: str) -> Optional[Dict[str, Any]]:
            async with sem:
                try:
                    ...
                    trade = GoldenConsensusEngine.analyze(frames, symbol, tickers[symbol])
                    return trade
                except Exception:
